...
def create_playlist(time_range, limit):
    ...
    if time_range == 'short_term':
        ...
        playlist_name = "Top tracks of the last 6 months as of {} {}".format(get_current_month(), datetime.datetime.now().year)
    else:
        playlist_name = "Top tracks over the last few years as of {} {}".format(get_current_month(), datetime.datetime.now().year)
    ...
    if token:
        ...
        # Get top 50 tracks from the chosen time range
    ...
    tweet_str = "Here's a spotify playlist of my most listened to songs in {}\n{}\n\nThis includes artists such as: ".format(turn_time_range_into_english_sentence(time_range), playlist_link_url)
...
def check_number(value):
    ...
    if ivalue <= 0 or ivalue > 100:
         raise argparse.ArgumentTypeError("%s is an invalid int value. The limit is between 1 and 100" % value)
    return ivalue

parser = argparse.ArgumentParser()
parser.add_argument("-t", "--timeframe", help="Creates a playlist of the top songs from the past month, 6 months or years.", choices=['short_term', 'medium_term', 'long_term'])
